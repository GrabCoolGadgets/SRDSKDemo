    ...
    if len(m.command) == 2:
        ...
        if "help" in m.command[1]:
            ...
            try:
                ...
                # Manually construct file_id and chat_id and recursively call start
                # This is a HACK to avoid database lookup.  If the IDs are incorrect, it WILL fail
                # Create a new message object and call the handler directly
                new_message = types.Message(
                    id=m.id + 1,  # Just needs to be unique
                    date=m.date,
                    chat=m.chat,
                    from_user=m.from_user,
                    text=f"/start {movie_name}"
                )
                new_message.command = ["start", f"{movie_name}"] # Set command properly
                await start(c, new_message) # Recursive call
                return
